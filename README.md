...
Povezati cRIO i sbRIO kontrolere (fizičko povezivanje uređaja preko odgovarajućih kablova/mreže).
U NI MAX-u proveriti da su oba uređaja prepoznata i dostupna na mreži.
Otvoriti LabVIEW projekat i, u Project Exploreru, dodati oba uređaja (cRIO i sbRIO) u stablo projekta.
Postaviti VI upravljačke aplikacije (automat) na cRIO, VI modela lifta na sbRIO.
Deploy-ovati biblioteku deljenih promenljivih (Shared Variable Library).
Pokrenuti VI-jeve redosledom: model (sbRIO) → upravljačka aplikacija (cRIO) → korisnički panel (PC).
